...
import { Api, useBox } from '@react-three/cannon'
...
export type ChassisRef = ReturnType<typeof Chassis>

export const Chassis = forwardRef<Api>(({ args = [2, 1.1, 4.7], mass = 500, children, ...props }, ref) => {
  const glass = useRef<Mesh>()
  const brake = useRef<Light>()
  const wheel = useRef<Mesh>()
  const needle = useRef<Mesh>()
  ...
  const [, api] = useBox(() => ({ mass, args, allowSleep: false, onCollide, ...props }), ref as any)
  ...
  useFrame((_, delta) => {
    ...
    if (brake.current && brake.current.material.color && brake.current.material.emissive) {
