...
import { angularVelocity, getState, mutation, position, rotation, useStore } from '../../store'
...
export const Chassis = forwardRef<Object3D, ChassisProps>(({ args = [2, 1.1, 4.7], mass = 500, children, ...props }, ref) => {
  ...
  const [reset, ready, camera, vehicleConfig] = useStore((s) => [s.reset, s.ready, s.camera, s.vehicleConfig])
  ...
  useFrame((_, delta) => {
    ...
    wheel.current.rotation.z = MathUtils.lerp(wheel.current.rotation.z, ctrl.left ? -Math.PI : ctrl.right ? Math.PI : 0, delta)
