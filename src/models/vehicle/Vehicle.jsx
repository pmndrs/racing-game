...
function VehicleAudio() {
  ...
  useFrame((state, delta) => {
    speed = mutation.speed
    controls = gameState.controls
    const { maxSpeed } = gameState.vehicleConfig

    boostAudio.current.setVolume(gameState.sound ? (controls.boost ? Math.pow(speed / maxSpeed, 1.5) + 0.5 : 0) * 5 : 0)
    boostAudio.current.setPlaybackRate(Math.pow(speed / maxSpeed, 1.5) + 0.5)
    engineAudio.current.setVolume(gameState.sound ? 1 - speed / maxSpeed : 0)
    accelerateAudio.current.setVolume(gameState.sound ? (speed / maxSpeed) * 2 : 0)
